...
class Cell():
    ...
    def draw(self, x1, y1, x2, y2):
        ...
        if self.has_left_wall:
            left_wall = Line(Point(self._x1, self._y1), Point(self._x2, self._y1))
            self._win.draw_line(left_wall)
        if self.has_right_wall:
            right_wall = Line(Point(self._x1, self._y2), Point(self._x2, self._y2))
            self._win.draw_line(right_wall)
        if self.has_top_wall:
            top_wall = Line(Point(self._x1, self._y1), Point(self._x1, self._y2))
            self._win.draw_line(top_wall)
        if self.has_bottom_wall:
            bottom_wall = Line(Point(self._x2, self._y1), Point(self._x2, self._y2))
